...
def is_heap_recursive(l, n=None, i=0, heap_type=HeapType.MAX):
	...
	if (leftchild < n): # if leftchild is present
		if l[leftchild] is None:	# if leaf reached, mark left is tree as True
			left_is_tree = True
		elif ((heap_type == HeapType.MAX and l[i] >= l[leftchild]) or	# if max, parent > child, else parent < child
	...
	if (rightchild < n):
		if l[rightchild] is None:	# if right reached, mark left is tree as True
			right_is_heap = True
		elif ((heap_type == HeapType.MAX and l[i] >= l[rightchild]) or
			(heap_type == HeapType.MIN and l[i] <= l[rightchild])):
...
def move_root_to_leaf_max_heap(l, i, n):
	...
	if leftchild >= n: # base condition (picking smaller index since we want to account for situation where there is only one child)
		return
	largestchild = leftchild; otherchild = rightchild
	if rightchild < n and l[largestchild] < l[rightchild]:	# we need 1st check rightchild is within the bounds and only then compare
		largestchild = rightchild; otherchild = leftchild
...
def heap_sort_using_insertion(l):
	...
	n = len(mh)
	for lasti in range(n-1,-1,-1): # remove operation from root to leaf where the size of heap reduces in each iteration
		...
		swap(mh, lasti, root)
		move_root_to_leaf_max_heap(l=mh, i=root, n=lasti) # 2.b. i represents root
		# print("tmp mh", mh)
	return mh
...
def heap_sort_using_heapify(l):
	...
	for i in range(parent_of_last_leaf, -1, -1):	# go backward right -> left (or leaft -> root) until top 0 (including top root=0)
...
def test_navigate_heap():
	...
	testcases = [
		...
		([50,30,20,10,11,5,None], HeapType.MAX, True),	# corner good case
		([50,30,20,10,11,-5,None], HeapType.MAX, True),	# corner good case
		([50,30,20,None,None,None,None], HeapType.MAX, True),	# corner good case
